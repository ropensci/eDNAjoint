...
#' NA_standards
#' @srrstatsNA {G1.2} I attempted to add lifecycle statements with the lifecycle package but was getting ROxygen errors when doing `usethis::use_lifecycle()`. It would be nice to get some guidance on the best way to do this.
...
#' @srrstatsNA {G5.4a} This software is not proposing a new algorithm and instead uses functions from the `rstan` package, so it is unclear to me if and how comparing to a C++ would be useful.
...
#' @srrstatsNA {G5.9, G5.9a} It is unclear to me how to add trivial noise to the data, since the expected data are all integers.
#' @srrstatsNA {G5.10,G5.11,G5.11a,G5.12} I have not implemented any extended tests
...
#' @srrstatsNA {BS1.3a, BS2.8} Since all my sampling functions uses `sampling` from `rstan`, it is unclear how the user could use results from previous runs to be used as starting points for a new run, although there is a way for the user to specify initial values (with an example in the vignette)
...
#' @srrstatsNA {BS2.13, BS2.14} This software uses `rstan`'s `sampling`, and while it seems possible to suppress all messages/warnings/progress bars, it is unclear how to suppress just progress bars or just warnings.
