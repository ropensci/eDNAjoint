...
jointModel <- function(data, cov = NULL, family = 'poisson',
  ...
  dna_ind <- unique(qPCR_all$L_ind)[!unique(qPCR_all$L_ind) %in% trad_ind]

  # subset qPCR data -- paired
  qPCR_all_trad <- qPCR_all[qPCR_all$L_ind %in% trad_ind,]
  L_match_trad <- as.data.frame(cbind(unique(qPCR_all_trad$L_ind),
                                      1:length(unique(qPCR_all_trad$L_ind))))
  colnames(L_match_trad) <- c('L_ind','L_unique')
  qPCR_all_trad <- dplyr::left_join(qPCR_all_trad,L_match_trad,by='L_ind')

  # subset qPCR data -- unpaired
  if(length(dna_ind)>0){
    qPCR_all_dna <- qPCR_all[qPCR_all$L_ind %in% dna_ind,]
    L_match_dna <- as.data.frame(cbind(unique(qPCR_all_dna$L_ind),
                                       1:length(unique(qPCR_all_dna$L_ind))))
    colnames(L_match_dna) <- c('L_ind','L_unique')
    qPCR_all_dna <- dplyr::left_join(qPCR_all_dna, L_match_dna, by='L_ind')
  } else {
    qPCR_all_dna <- as.data.frame(matrix(NA,nrow=0,ncol=4))
    colnames(qPCR_all_dna) <- c('L_ind','n_N','n_K','L_unique')
  ...
  if(all(!is.null(cov))){
    ...
    site_mat <- rep(1,
                    length(unique(qPCR_all_trad$L_ind))+
                      length(unique(qPCR_all_dna$L_ind)))
  }
  ...
  model_data <- list(
    n_S = nrow(qPCR_all_trad),
    S_dna = nrow(qPCR_all_dna),
    n_C = nrow(count_all),
    L_ind = qPCR_all_trad$L_unique,
    L_dna = qPCR_all_dna$L_unique,
    R_ind = count_all$R_ind,
    Nloc_dna = length(unique(qPCR_all_dna$L_ind)),
    Nloc_trad = length(unique(qPCR_all_trad$L_ind)),
    trad_ind = trad_ind,
    dna_ind = as.array(dna_ind),
    n_E = count_all$count,
    n_N = qPCR_all_trad$n_N,
    n_K = qPCR_all_trad$n_K,
    N_dna = qPCR_all_dna$n_N,
    K_dna = qPCR_all_dna$n_K,
