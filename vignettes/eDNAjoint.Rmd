...
```{r, echo=FALSE}
symbol <- c("$\\mu_i$","$p_{10}$","$q_k$","$\\beta$","$\\alpha_n$", "$\\phi$")
name <- c("mu", "p10", "q", "beta","alpha", "phi")
description <- c(paste0("Vector of expected catch rate at site, i. If multiple traditional gear types are used, mu refers to the expected catch rate of the reference gear type."),
...
knitr::kable(param_table,
             caption = "Table 1. Parameters included in joint model, including symbols, names, and descriptions.")
...
6. q_ref: integer indicating the reference gear type. Since we've chosen 1, $\mu$ now describes the expected catch rate of gear type 1, and the catchability of all other gear types are scaled relative to gear type 1.
...
To further highlight these results, we can use `detection.calculate()` to find the units of survey effort necessary to detect presence of the species. This function is finding the median number of survey units necessary to detect species presence if the expected catch rate, $\mu$ is 0.1, 0.5, or 1. Again, $\mu$ now represents the catch rate specific to gear type 1.

```{r}
detection.calculate(greencrab.fit.q.negbin, mu = c(0.1,0.5,1), probability = 0.9)
```

We can see that it takes 27 eDNA samples, 25 reference trap samples (gear type 1), and 31 non-reference trap samples (gear type 2) to detect green crab presence with 0.9 probability if the expected catch rate is 0.1.
...
4. q_ref: integer indicating the reference gear type. Since we've chosen 1, $\mu$ now describes the expected catch rate of gear type 1, and the catchability of all other gear types are scaled relative to gear type 1.

More parameters exist to further customize the MCMC sampling, but we'll stick with the defaults.

```{r, eval = FALSE}
#run the traditional model model with catchability coefficients
greencrab.traditional <- traditionalModel(data = greencrabData, family = 'negbin', q = TRUE, q_ref = 1) 
```
   

## The model

Below is a representation of the full model used in *eDNAjoint*, including all model variations. Note that inclusion of catchability coefficients, *q*, and the regression with site-level covariates, $\alpha$ (Eq. 3), are optional in implementation with *eDNAjoint*.
...
1. a negative binomial distribution with expected species catch rate, $\mu_i$, and an overdispersion parameter, $\phi$ (Equation 1.1)
2. a poisson distribution with expected species catch rate, $\mu_i$ (Equation 1.2).

\begin{equation}
\tag{Eq. 1.1}
Y_{i,j,k} \sim NegativeBinomial(q_k * \mu_i, \phi) 
\end{equation} 

\begin{equation}
\tag{Eq. 1.2}
Y_{i,j,k} \sim Poisson(q_k * \mu_i) 
\end{equation} 

The probability of a true positive eDNA detection, $p_{11}$, at site *i*, is a function of expected species catch rate, $\mu_i$ and scaling coefficient $\beta_i$ (Equation 2).

\begin{equation}
\tag{Eq. 2}
p_{11,i} = \frac{\mu_i}{\mu_i + e^{\beta_i}}
\end{equation} 

The scaling coefficient $\beta_i$ relates the sensitivity of eDNA sampling to the expected species catch rate and is a function of site-level covariate coefficients, $\alpha_n$ and site-level covariate data, $A_{i,n}$ (Equation 3).
...
The total probability of eDNA detection at site, $p_i$, is the sum of the probability of a true positive eDNA detection, $p_{11}$, and the probability of a false positive eDNA detection, $p_{10}$ (Equation 4).
...
The number of positive quantitative PCR (qPCR) eDNA detections, K, out of the number of trials, N, in eDNA water sample *m* at site *i* is drawn from a binomial distribution, with a probability of success on a single trial, $p_i$. (Equation 5).
...
The models that can be run with *eDNAjoint* use Bayesian inference for parameter estimation. The models are specified in the probabilistic programming language, [Stan](https://mc-stan.org/rstan/), which uses Hamilton Monte Carlo to obtain posterior simulations. For this reason, all the models fit using *eDNAjoint* are of the 'stanfit' class and can be analyzed and manipulated with functions in the *rstan* package, in addition to the functions outlined above.
