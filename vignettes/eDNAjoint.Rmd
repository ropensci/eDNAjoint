...
```{r}
#' @srrstats {BS1.2a, BS1.2b} Detailed vignette with examples and explanations here
```
...
2. seeds used in MCMC (`goby.fit1$seeds`).
3. initial values used for each chain in MCMC (`goby.fit1$inits`)
...
#' @srrstats {BS1.4,BS4.3,BS6.2} Example of convergence checking here and plotting posterior samples

# this will plot the MCMC chains for p10 and mu at site 1
mcmc_trace(extract(goby.fit1$model, permuted = FALSE), pars = c('p10', 'mu[1]'))
...
```{r}
detectionCalculate(goby.fit1$model, mu=c(0.1,0.5,1), cov.val = c(0,0), probability = 0.9)
```
...
detectionCalculate(goby.fit1$model, mu=c(0.1,0.5,1), cov.val = c(0.5,0), probability = 0.9)
```
...
```{r}
detectionCalculate(goby.fit1$model, mu=c(0.1,0.5,1), cov.val = c(0,0.5), probability = 0.9)
```
...
detectionPlot(goby.fit1$model, mu.min=0.1, mu.max =1, cov.val = c(0,0), probability = 0.9)
```
...
for(i in 1:n.chain){
  inits[[i]] <- list(
    # length should equal the number of sites (dim(data$count)[1]) for each chain
    mu = stats::runif(dim(data$count)[1], 0.01, 5), 
...
# this will plot the MCMC chains for p10 and mu at site 1
mcmc_trace(extract(greencrab.fit.q.negbin$model, permuted = FALSE), pars = c('p10', 'beta'))
...
```{r}
detectionCalculate(greencrab.fit.q.negbin$model, mu = c(0.1,0.5,1), probability = 0.9)
```
...
```{r, fig.width = 7}
detectionPlot(greencrab.fit.q.negbin$model, mu.min = 0.1, mu.max = 1, probability = 0.9)
```
